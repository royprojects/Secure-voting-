...
def register(request):
    time = get_vote_auth()
    format = "%d/%m/%Y at %H:%M:%S %Z%z"
    if time and time[0].end < datetime.datetime.now(datetime.timezone.utc):
...
def signin(request):
    time = get_vote_auth()
    if time and time[0].end > datetime.datetime.now(datetime.timezone.utc) and time[0].start < datetime.datetime.now(datetime.timezone.utc):    
        if request.method == 'POST':
        ...
        if time and time[0].end < datetime.datetime.now(datetime.timezone.utc):
        ...
        elif time and time[0].start > datetime.datetime.now(datetime.timezone.utc):
            ...
            context = {
                'fail': "Voting schedule not available."
            }
        return render(request,'poll/failure.html',context)
